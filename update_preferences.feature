...
Feature: As A Moodle User,
  ...
  Scenario Outline: Login with administrator permissions Feature
    ...
    And I click on "//div/span[contains(.,'Dashboard')]" "xpath_element"
    And I should see "Acceptance test site"
    And I follow "Preferences" in the user menu
    ...
    And I should see "Forum preferences"
    And I set the field with xpath "//select[@id='id_maildigest']" to "Complete (daily email with full posts)"
    And I set the field with xpath "//select[@id='id_autosubscribe']" to "No: don't automatically subscribe me to forum discussions"
    And I set the field with xpath "//select[@id='id_trackforums']" to "Yes: highlight new posts for me"
    And I press "Save changes"
    And I should see "Acceptance test site"
    And I follow "Preferences" in the user menu
