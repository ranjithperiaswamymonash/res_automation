@res_automation @resource @create_url_resource
Feature: Create a moodle url resource
  In order to create a url resource
  As an admin
  I need to be create url resource so that students can use it in course.

  @javascript
  Scenario: Create a moodle url resource
    ...
    And I click on "//span[@class='typename' and text()='URL']" "xpath_element"
